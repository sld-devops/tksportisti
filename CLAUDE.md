...
`morning` / `afternoon` / `evening` are drawn in exactly three places in `styles.css` and must agree: `.tod-badge.tod-*` (the badge on a card), `.time-of-day-buttons .add-day-button[data-tod=*]` (the coach's "Ieplānot no rīta" buttons in an empty day), and `.tod-radio-label.tod-*` (the radios in the restrictions form and in the self-log form). Rīts is `#fff7d6`/`#5a4a00`, Pusdiena `#e0f2fe`/`#075985`, Vakars `#fce7f3`/`#9b1c47`.
